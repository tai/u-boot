...
#define RSA_KEY_N_SIZE                                  2048    // RSA2048 supported
#define RSA_KEY_E_SIZE                                  23      // 24 bit E
#define RSA_E_ARRAY_SIZE                                (RSA_KEY_E_SIZE > 23 ? 3 : 1)
#define RSA_KEY_SIZE                                    (RSA_KEY_N_SIZE * 2 + (RSA_KEY_N_SIZE >> 1))    // try 2*n + 4*32
...
typedef struct
{
    ...
    int max_size;       /*!<  maximum array size */
    t_int p[(RSA_KEY_SIZE>>3)/sizeof(t_int)];           /*!<  pointer to limbs  */
}mpi;
