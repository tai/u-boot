 ...
 */
static inline int cn100_is_zero_ether_addr(const u8 *addr)
{
 ...
 */
static inline int cn100_is_multicast_ether_addr(const u8 *addr)
{
	return (0x01 & addr[0]);
}
 ...
 */
static inline int cn100_is_valid_ether_addr(const u8 *addr)
{
	/* FF:FF:FF:FF:FF:FF is a multicast address so we don't need to
	 * explicitly check for it here. */
	return !cn100_is_multicast_ether_addr(addr) && !cn100_is_zero_ether_addr(addr);
}
...
int cn100_eth_readmac(bd_t * bd)
{
    if (getenv ("ethaddr"))
    {
        ...
        {
		    bd->bi_enetaddr[0] = enetaddr[0];
            bd->bi_enetaddr[1] = enetaddr[1];
            bd->bi_enetaddr[2] = enetaddr[2];
            bd->bi_enetaddr[3] = enetaddr[3];
            bd->bi_enetaddr[4] = enetaddr[4];
            bd->bi_enetaddr[5] = enetaddr[5];
		}
	}
    else
    {
#if defined(CONFIG_CMD_EEPROM)
                eeprom_read(CONFIG_SYS_I2C_EEPROM_ADDR, CONFIG_CNC1800L_ETHADDR_EEPROM_OFFSET, ((u8 *) bd->bi_enetaddr),6);
#elif !defined(CFG_CNC1800L_NAND_BOOT) && defined(CFG_CNC1800L_ETHADDR_FROM_NOR_FLASH)
	...
	if (!cn100_is_valid_ether_addr(bd->bi_enetaddr))
    {
		((u8 *) bd->bi_enetaddr)[0] = 0x00;
		((u8 *) bd->bi_enetaddr)[1] = 0x1a;
		((u8 *) bd->bi_enetaddr)[2] = 0xcc;
		((u8 *) bd->bi_enetaddr)[3] = 0x00;
		((u8 *) bd->bi_enetaddr)[4] = 0x00;
		((u8 *) bd->bi_enetaddr)[5] = 0x01;
	}
    eth_setenv_enetaddr("ethaddr", ((u8 *)bd->bi_enetaddr));
