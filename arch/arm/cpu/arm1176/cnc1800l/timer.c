...
ulong get_timer(ulong base)
{
	ulong current_time = tick_to_time(get_ticks());
#ifdef CONFIG_LED_BLINK
	if(current_time - blink_base > CONFIG_SYS_HZ/CONFIG_LED_BLINK_HZ){
		blink_base = current_time;
		blink_toggle = !blink_toggle;
		gpio_hw_set_direct(CONFIG_LED_BLINK_GPIO, 1);
		gpio_hw_write(CONFIG_LED_BLINK_GPIO, blink_toggle);
	}
#endif
	return current_time - base;
}
