...
int ext_fs_read(char *devname, int dev,int *sz)
{
	unsigned long part = 1;
	char *ep;
	struct ext_filesystem *fs;
	...
	ext4_dev_desc = get_dev(devname, dev);
	...
	*sz = ext4fs_open("misc.img");
	if (*sz < 0) {
		printf("** File not found %s\n","misc.img");
		ext4fs_close();
		return -1;
	}
	//support maximal 256M image
	*sz=ext4fs_read( CONFIG_USB_UPGRADE_DATA_ADDR,*sz);
	if (*sz <= 0) {
