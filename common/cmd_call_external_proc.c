...
#define EXTERNAL_PROCS_MAX_ARGC 9
...
#define PIC_PROC_HAS_SEPERATE_SPACE
  ...
  for(i=0;i<sizeof(this_code_attr)/sizeof(struct external_code_attr);i++)
  {
    ...
    if(app[0] == magic[0] && app[1] == magic[1]) 
    {
	  ...
	  file_crc=crc32 (0, (uchar *)this_code_attr[i].addr, this_code_attr[i].file_size);
	  this_code_attr[i].file_crc=file_crc;
  ...
  if(0==strcmp(argv[0],"list"))
  {
    for(i=0;i<sizeof(this_code_attr)/sizeof(struct external_code_attr);i++)
    {
      if(0!=this_code_attr[i].key_word)
      {
        ...
        printf("%d:key_word=[%c%c%c%c],addr=[0x%X],file_crc=[0x%X],file_size=[0x%X],context_size=[0x%X].\n",
          i,p[3],p[2],p[1],p[0],this_code_attr[i].addr,this_code_attr[i].file_crc,this_code_attr[i].file_size,this_code_attr[i].context_size);
      }
      else
      {
        break;
  ...
  else
  {
    ...
    for(i=0;i<sizeof(this_code_attr)/sizeof(struct external_code_attr);i++)
    {
      if(key_num == this_code_attr[i].key_word)
      {
		...
		if(CONFIG_UBOOT_EXTERNAL_PROC_LEN<this_code_attr[i].context_size)
		{
			printf("ERROR:The context exceed CONFIG_UBOOT_EXTERNAL_PROC_LEN bytes, it's too big to execute in PIC proc buffer.\n");
			break;
		}
        
#ifndef PIC_PROC_HAS_SEPERATE_SPACE        
        app_ori_buf=(unsigned char *)malloc(((this_code_attr[i].context_size+(sizeof(unsigned int)-1)))&(~(sizeof(unsigned int)-1)));
        if(NULL==app_ori_buf)
        {
          printf("%s:malloc failed,size is 0x%X bytes.\n",__FUNCTION__,this_code_attr[i].context_size);
          break;    
        }
        app_aligned_buf=(unsigned char *)(((int)(app_ori_buf+(sizeof(unsigned int)-1)))&(~(sizeof(unsigned int)-1)));
#else        
        app_aligned_buf=(unsigned int*)CONFIG_ALIGNED_PIC_PROC_MEM_BASE;
#endif
        memcpy(app_aligned_buf,this_code_attr[i].addr,this_code_attr[i].file_size);
		//disable icache
		if(0!=icache_status())
		{
			icache_flag=1;
