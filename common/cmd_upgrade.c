...
#ifdef ENABLE_IP_UPDATE
int ReadNetHdBinFromUpdatePartition(char *ip_update_addr)
{
	int status = 0 ;
	char* cmd1[] = { "nand" , "read" , FILE_MEM_ADDR_STRING, ip_update_addr , "0x3e00000" };
	...
	{
		printf("ERROR: unable to write NAND flash for cavm18_nethd.bin/ cavm18_nethd.bin ip update\n");
	}
}
#endif
...
#if	(CONFIG_TARGET_NAME == CNC1800L_TMS_512_256)
	char* cmd1[] = { "nand" , "erase", "0x04400000" , "0x04000000" };
#else
...
int ProgramNetHdToNandFlash(char *filename)
{
	...
	do {
		if (0 == strcmp(ext, SECURE_IMAGE_EXTENSION)) {
			...
			char filesize[16]={0};
#if	(CONFIG_TARGET_NAME == CNC1800L_TMS_512_256)
			char* cmd1[] = { "nand" , "write" , FILE_MEM_ADDR_STRING , "0x04400000",  & filesize[0] };
#else
			char* cmd1[] = { "nand" , "write" , FILE_MEM_ADDR_STRING , "0x02400000",  & filesize[0] };
#endif			
#ifndef ENABLE_IP_UPDATE			
			strncpy(filesize , getenv("filesize") , sizeof(filesize) ) ;
#else
			strcpy(filesize,"0x3e00000");
#endif			
...
#ifdef ENABLE_IP_UPDATE
	int  ip_update = 0;
	char ip_update_addr[16]={0};
	char ip_update_option[8]={0};
#endif
...
/****************************************************************
 * in ordr to support ip_update feature , format is as following:
 * upgrade  nethd        ip_update  addr option
 * upgrade  uboot force  ip_update  addr option
 * addr    : address of update nand partition
 * option  : 0 --- non-secure  , .bin
 *         : 1 --- secure image  .sbin 
 *
*****************************************************************/
#ifdef ENABLE_IP_UPDATE	
	else if(0 == strcmp(argv[2],"ip_update")){	
		ip_update = 1;
		strncpy(ip_update_addr,argv[3],16);
		strncpy(ip_update_option,argv[4],8);
	}
	else if(0 == strcmp(argv[3],"ip_update")){
		ip_update = 1 ;
		strncpy(ip_update_addr,argv[4],16);
		strncpy(ip_update_option,argv[5],8);
	}	
...
#ifdef ENABLE_IP_UPDATE		
		if(ip_update == 1){
			//argv[3] : address of update nand partition start address
			status = ReadNetHdBinFromUpdatePartition(ip_update_addr);		
			//argv[4] : update option : 0--insecure 1 --secure
			if(strcmp(ip_update_option , "0")   == 0 ){
				filename = INSECURE_IMAGE_FILENAME;
			}
			else
				filename = SECURE_IMAGE_FILENAME;
			if(status != 0)
			{
				printf("ERROR: fail to read NAND update partition\n");
				goto exit2; 
			}
...
U_BOOT_CMD(
	upgrade,	6,	0,	do_upgrade,
	"Utility to upgrade uboot or nethd image\n",
	"\t  upgrade spiuboot|uboot|nethd [force] [filename] / upgrade spiuboot|uboot|nethd [force] ip_update addr option   \n"
	...
	"\t  ip_update indicates that nethd image is from update partion \n"
	"\t  addr indicates that start address of update partion \n"
	"\t  option indicates image type : 0 --non-secure 1--secure \n"
	);
