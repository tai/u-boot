...
static int sec1800l_md5sum(uint32_t aImgDramAddr,  uint32_t aImgLen, char *cmp_md5sum)
{
	...
	if (memcmp (output, cmp_md5sum, 16) != 0) {
        ...
        printf("\n on %u bytes of data at memory address 0x%08x\n", aImgLen, aImgDramAddr);
	}
...
int ImgRsa_Verify (uint32_t aImgAddress, uint32_t aLen, uint32_t asignAddress)
{
	...
	printf (" Verifying Image RSA signature... ");
...
int Img_Authenticate (FlashImageInfo_t *hdr_p, int distImgType)
{
	...
	if (hdr_p->mHdrType == BLOB_IDENTIFIER ||
		...
		if (chk_crc32 == hdr_p->mCrc32) {
			...
			debug ("\n input hash dump: "); hex_print (inhash, 32);
			...
			memcpy (hash, (uint8_t *)( Hash_Buf_Offset), 16);
		        debug ("\n 1st pass hash dump (16byte): "); hex_print (hash, 16);

			/* copy inhash's last 26 byte (keyB) into hash last 16 byte index */
			memcpy (hash+16, (inhash + 16), 16);

                        debug ("\n 2nd Encryption pass hash dump (16byte): "); hex_print (hash+16, 16);

                        debug ("\n 1st/2nd encryption pass hash dump (32 byte): "); hex_print (hash, 32);

			/* compare 32 byte has with input Image */
			if (memcmp(hash, inhash, 32) != 0) {

				printf ("Error : Hashing failure!\n");
...
i			dontcare = measure_time_ms(&crypto_marker);
			...
			/* copy last 16byte hash store in local hash bufffer */
			memcpy (hash, (uint8_t *)( Hash_Buf_Offset), 16);
		        debug ("\n 1st encryption pass hash dump (16 byte): "); hex_print (hash, 16);
...
FAIL_SAFE:
	...
	if (fail_safe_flag == 1) {
		...
		printf (" Field Img Authentication failed ... Look for Factory Img \n");
	...
	debug (" mHdrType : 0x%x mSourceAddressInFlash : 0x%x mDestinationAddressInDram : 0x%x \
			App Image image mLength : %d siglen : %d ActiveImage flag : %d 		\
			mVersion : %s Crc32 : 0x%x \n", blobImgHdr->mHdrType, blobImgHdr->mSourceAddressInFlash,
							blobImgHdr->mDestinationAddressInDram, blobImgHdr->mLength,
							blobImgHdr->mSignLen, blobImgHdr->mActiveImage,
				...
				uint32_t actImgType)
{
	...
	// condition check for memory boundary so that ImgLen shouldn't be greater than 32 MB
	if (ImgLen_aligned >= (32*1024*1024)) {

		printf (" Image Size greater than 32 MByte.. Exiting \n");
		goto Error1;
	}
...
static int find_needle_in_haystack (unsigned char *needle, int len, unsigned char *haystack, int limit)
{
    int i;
    unsigned char c;
    int state = 0;
